...
public class Graph : MonoBehaviour {
	...
	static GraphFunction[] functions = { SineFunction, Sine2DFunction, MultiSineFunction, MultiSine2DFunction, Ripple, 
											Cylinder, Sphere, Torus, StrangeFunction };
	Transform[] points;
	...
	static Vector3 StrangeFunction(float x, float z, float t) {
		Vector3 p;
		p.x = x;
		p.y = Mathf.Sin(pi * (x * t));
		p.z = z;
	...
	static Vector3 Torus(float u, float v, float t) {
		Vector3 p;
		float r1 = 0.65f + Mathf.Sin(pi * (2f * u + t)) * 0.1f;
		float r2 = 0.2f + Mathf.Sin(pi * (3f * v + t)) * 0.05f;
